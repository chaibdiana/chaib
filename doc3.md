...
\# \=========================================================  
\# Parâmetro DINÂMICO (regra de negócio)  
\# \=========================================================

def infer\_parametro\_dinamica(modelo\_nome: Any) \-\> Optional\[str\]:  
...
def main() \-\> None:  
    ...
    \# BASE DINÂMICA  
    \# \-------------------------  
    base\["parametro\_dinamica"\] \= base\["modelo\_nome"\].apply(infer\_parametro\_dinamica)

    base\_dyn \= base\[  
        base\["parametro\_dinamica"\].notna() &  
        base\["score\_item"\].notna()  
    \].copy()

    mask \= (  
        (base\_dyn\["parametro\_dinamica"\] \== "LGD") & (base\_dyn\["parametro"\] \== "LGD")  
    ) | (  
        (base\_dyn\["parametro\_dinamica"\] \== "EAD/FCC") & (base\_dyn\["parametro"\].isin(\["EAD", "FCC"\]))  
    ) | (  
        (base\_dyn\["parametro\_dinamica"\] \== "PD") & (base\_dyn\["parametro"\] \== "PD")  
    ) | (  
        (base\_dyn\["parametro\_dinamica"\] \== "SCORING") &  
        (base\_dyn\["parametro"\].isin(\["SCORING", "RR", "BEHAVIOUR"\]))  
    )

    base\_dyn \= base\_dyn\[mask\]

    \# \-------------------------  
    \# TABELA DINÂMICA  
    \# \-------------------------  
    group\_cols \= \[  
        "ano\_validacao", "modelo\_nome", "segmento",  
        "parametro\_dinamica", "modelo\_utilizado", "mrm\_codes"  
    \]

    scores \= (  
        base\_dyn  
        .groupby(group\_cols, dropna=False)  
        .agg(score\_pct=("score\_item", "mean"))  
        .reset\_index()  
    )

    scores\["score\_pct"\] \*= 100

    pivot \= scores.pivot\_table(  
        index=group\_cols\[1:\],  
        columns="ano\_validacao",  
        values="score\_pct"  
    ).reset\_index()

    if 2024 in pivot.columns:  
        pivot.rename(columns={2024: "score\_pct\_2024"}, inplace=True)  
    if 2025 in pivot.columns:  
        pivot.rename(columns={2025: "score\_pct\_2025"}, inplace=True)

    if {"score\_pct\_2024", "score\_pct\_2025"}.issubset(pivot.columns):  
    ...
    with pd.ExcelWriter(out\_path, engine="openpyxl") as writer:  
        ...
        pivot.to\_excel(writer, index=False, sheet\_name="tabela\_dinamica\_modelos")
